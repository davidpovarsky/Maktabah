...
def compress_zstd(source: Path, destination: Path) -> None:
    ...
    with source.open("rb") as input_handle, destination.open("wb") as output_handle:
        # A fixed worker count keeps frame boundaries identical on every runner.
...
def main() -> None:
    ...
    for key, actual in (
        ("databaseBytes", provenance["sourceDatabaseBytes"]),
        ("databaseSHA256", provenance["sourceDatabaseSHA256"]),
    ):
        if expected_source.get(key) != actual:
            raise RuntimeError(
                f"source {key} mismatch: expected {expected_source.get(key)}, got {actual}"
            )
