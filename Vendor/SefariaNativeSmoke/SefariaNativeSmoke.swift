...
enum SefariaNativeSmoke {
    static func main() async throws {
        ...
        await check("catalog", failures: &contractFailures) {
        ...
        await check("texts-v3", failures: &contractFailures) {
        ...
        await check("complex-navigation", failures: &contractFailures) {
        ...
        await check("ref-navigation", failures: &contractFailures) {
        ...
        await check("search-wrapper", failures: &contractFailures) {
        ...
        var packages: [SefariaPackageManifestEntry]?
        await check("package-manifest", failures: &contractFailures) {
            let result = try await client.get([SefariaPackageManifestEntry].self,
                url: config.readonlyURL(path: "\(root)/packages.json"))
            guard !result.isEmpty else { throw LibraryBackendError.invalidResponse("empty package manifest") }
            packages = result
        }

        if let packages {
            await check("complete-library-manifest", failures: &contractFailures) {
            ...
            await check("smallest-package-download", failures: &contractFailures) {
        ...
        print("Sefaria network smoke passed using all \(bundlePaths.count) bundle(s) from package \(package.en)")
            ...
            contractFailures.append("complete-library-manifest: NOT RUN (package manifest unavailable)")
            contractFailures.append("smallest-package-download: NOT RUN (package manifest unavailable)")
        }

        if !contractFailures.isEmpty {
            print("Sefaria live contract failures (\(contractFailures.count)):")
            contractFailures.forEach { print("- \($0)") }
